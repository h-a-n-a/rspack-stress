...
RUN git clone -b v1.8.2 --depth 1 https://github.com/microsoft/mimalloc /mimalloc && \
    cd mimalloc && mkdir build && \
    cd build  && cmake .. && make

FROM node:20.11.1-alpine3.18

COPY --chown=root:root --from=builder /mimalloc/build/libmimalloc.so.1 /lib/libmimalloc.so
